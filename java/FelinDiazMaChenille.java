...
public class FelinDiazMaChenille{
	...
	public FelinDiazMaChenille(int longueur, int epaisseur, float angleVision, Color c){ 
		...
		for(int i=longueur-2 ; i>=0 ; i--){
			corps[i] = new FelinDiazCercle( (float)((this.tete.xMax/4)-(epaisseur/2)*i), this.tete.yMax/4, epaisseur );
		}
	    }             

	public float getAngleDeVision(){
		return this.tete.getAngleDeVision();
	}

	public void setAngleDeVision(float a){
		this.tete.setAngleDeVision(a);
	}                                                       
